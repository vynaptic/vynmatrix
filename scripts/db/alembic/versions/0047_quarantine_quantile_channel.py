"""Quarantine QuantileChannel before corrected-version validation.

Revision ID: 0047_quarantine_quantile_v1_2
Revises: 0046_quarantine_vrpt_v1_2
"""
...
revision = "0047_quarantine_quantile_v1_2"
down_revision = "0046_quarantine_vrpt_v1_2"
branch_labels = None
depends_on = None


_STRATEGY_ID = "quantile_channel_v1"
_FEEDBACK_NOTE = (
    "[strategy-quarantine] Expired before corrected-version validation; "
    "historical feedback preserved."
)


def _converge_quarantined_state() -> None:
    """Reject attributable in-flight work, then remove execution authority."""
    ...
    op.execute(
        ...
        BEGIN
            SELECT
                ...
                + (SELECT COUNT(*)
                   ...
                   WHERE decisions.status IN ('pending', 'executing')
                     AND (
                         bindings.strategy_id = '{_STRATEGY_ID}'
                         OR context.strategy_id = '{_STRATEGY_ID}'
                         OR (
                ...
                + (SELECT COUNT(*)
                   FROM outbox_events
                   WHERE (
                       payload::JSONB ->> 'strategy_id' = '{_STRATEGY_ID}'
            ...
            IF unresolved_count > 0 THEN
                RAISE EXCEPTION
                    'Cannot quarantine {_STRATEGY_ID}: % non-terminal work items remain',
                    unresolved_count;
            END IF;

            -- Positions do not carry strategy attribution, so any holding
            -- blocks removal of this strategy runtime.
            ...
            IF nonzero_position_count > 0 THEN
                RAISE EXCEPTION
                    '% positions require reconciliation before strategy quarantine',
        ...
        BEGIN
            ...
            IF actionable_count <> 0 THEN
                RAISE EXCEPTION
                    'QuantileChannel quarantine left % actionable records',
                    actionable_count;
            END IF;
        END $$;
        """
...
def upgrade() -> None:
    _converge_quarantined_state()


def downgrade() -> None:
    # A schema rollback must not restore candidate execution authority.
    _converge_quarantined_state()
