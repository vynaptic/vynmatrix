"""Retire LiquidityLeadersBasket without deleting historical lineage.

Revision ID: 0098_retire_liquidity_leaders
Revises: 0097_binding_entry_cash_buffer
"""
...
revision = "0098_retire_liquidity_leaders"
down_revision = "0097_binding_entry_cash_buffer"
branch_labels = None
depends_on = None


_RETIRED_STRATEGY_ID = "liquidity_leaders_basket_v1"
...
def _converge_retired_state() -> None:
    ...
    op.execute(
        ...
        BEGIN
            ...
            -- Positions do not carry strategy attribution, so every open
        ...
        BEGIN
            ...
            IF actionable_count <> 0 THEN
                RAISE EXCEPTION
                    'LiquidityLeadersBasket retirement left % actionable records',
                    actionable_count;
            END IF;
        END $$;
        """
