...
> **Keep in sync with [AGENTS.md](AGENTS.md).** The two files are intentionally
> near-identical — Claude Code reads this one, Codex reads `AGENTS.md`. They may differ
> *only* where each names its own tool and its counterpart file: the title, the line
> above, and this note. Any other edit here must be applied to `AGENTS.md` in the same
