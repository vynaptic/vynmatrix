...
For the other platform, use the [macOS/Linux guide](SETUP_MAC_LINUX.md). The
source is available under [LICENSE](LICENSE) for personal, noncommercial use;
see [NOTICE](NOTICE) for attribution and third-party-material boundaries. These
steps do not authorize deployment or live orders.
...
Use PowerShell from the repository root. The database lifecycle uses the same `vmdev db` commands on every platform. If your local script policy blocks activation, use an approved policy for your machine; do not disable system safeguards.
