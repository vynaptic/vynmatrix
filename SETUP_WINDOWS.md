...
For the other platform, use the [macOS/Linux guide](SETUP_MAC_LINUX.md). The source is not yet open-source:
...
Use PowerShell from the repository root. Bash-only helpers such as `scripts/db/migrate_and_seed.sh` run inside the declared Docker services or through WSL/Git Bash. If your local script policy blocks activation, use an approved policy for your machine; do not disable system safeguards.
...
```powershell
$env:GITHUB_OWNER = "YOUR_GITHUB_OWNER"
git clone "https://github.com/$env:GITHUB_OWNER/vynmatrix.git"
cd vynmatrix
```
...
```powershell
$env:STRATEGY_LIST = "SwingHighLowPMO"
...
Reactivate the environment in a new terminal with `.\.venv-dev\Scripts\Activate.ps1`. Run focused
