...
| `signals.ingested`, `signals.scored`, `execution.results` and `feedback.ready` are enqueued unconditionally, claimed by the relay, and handed to `NoOpEventPublisher`; nothing consumes them | `pipeline_events.py` (deleted by this spec); [execution_persistence.py](../../../apps/execution_engine/execution_engine/execution_persistence.py); [paper_order_lifecycle.py](../../../apps/execution_engine/execution_engine/paper_order_lifecycle.py); [engine.py](../../../apps/feedback_loop_engine/feedback_loop_engine/engine.py); [main.py](../../../apps/scoring_engine/scoring_engine/main.py) `_build_event_publisher` |
...
| --- | --- | --- |
| `signals.ingested`, `signals.scored` | `pipeline_events.py` (deleted by this spec) `CanonicalSignalEvent`, `ScoredSignalEvent` | `canonical_signals`, `asset_scores`, `decision_contexts`, `execution_decision_logs` |
